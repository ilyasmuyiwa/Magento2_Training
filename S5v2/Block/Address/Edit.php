...
namespace Training\S5v2\Block\Address;
use Magento\Customer\Block\Address\Edit as OriginalEdit;
...
class Edit extends OriginalEdit
{
     ...
     * @param \Training\S5v2\Model\Source\AddressType $addressType
     * @param array $data
     *
    ...
    {
        $addressType = $this->getAddress()->getCustomAttribute('address_type');
        return $addressType === null ? '' :$addressType->getValue();
    }

    public function getAddressTypeOptions()
    {
        $arr = [];
        foreach ($this->addressType->getAllOptions() as $options){
